...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    io.Fonts->AddFontDefault();
    ImFont* mainFont = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Verdana.ttf", 20.0f);
...
bool IsInTitleBar(HWND hwnd, LPARAM lParam)
{
    ...
    if (pt.y <= rect.top + titleHeight)
    {
        // ignore the right side where the X button is
        int rightPadding = 50; // width reserved for buttons on right

        if (pt.x >= rect.right - rightPadding)
            return false; // let buttons receive clicks
